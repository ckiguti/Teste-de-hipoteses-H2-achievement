#' ---
#' title: "ANCOVA test for `fss`~`dfs`+`scenario`*`achievement`"
...
wid <- "ID"
covar <- "dfs"
between <- c("scenario","achievement")
dvs <- c("fss")
names(dvs) <- dvs
...
#' 
## ---- echo=FALSE-------------------------------------------------------------------------------------------------------------------------------
car::Boxplot(`fss` ~ `scenario`*`achievement`, data = dat[["fss"]], id = list(n = Inf))
...
## ---- echo=FALSE-------------------------------------------------------------------------------------------------------------------------------
for (dv in dvs) {
  car::Boxplot(`dv` ~ `scenario`*`achievement`, data = sdat[[dv]] %>% cbind(dv=sdat[[dv]][[dv]]), id = list(n = Inf))  
}
...
#' ### Textual Report
#' 
#' After controlling the linearity of covariance "dfs", ANCOVA tests with independent between-subjects variables "scenario" (gamified, non.gamified) and "achievement" (lower, upper) were performed to determine statistically significant difference on the dependent varibles "fss". For the dependent variable "fss", there was not statistically significant effects.
#' 
#' 
#' 
#' 
